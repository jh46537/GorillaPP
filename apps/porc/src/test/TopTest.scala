...
class TopTests(c: Top) extends gTester[Top](c) {
  ...
  val numThreads = 16
  val numOfInputs = 64
  val numOfOutputs = numThreads
  var tags = Array.fill(numThreads)(BigInt(0))

  while(cycles < 2500 && (sourced < numOfInputs || sinked < numOfOutputs)) {
    // Read packets from file
    // val word = lines(sourced).map(_.toLong)
    val word0 = BigInt(sourced*4)
    val word1 = BigInt(sourced*4+1)
    val word2 = BigInt(sourced*4+2)
    val word3 = BigInt(sourced*4+3)
    val bigWord = (word3 << 96) + (word2 << 64) + (word1 << 32) + word0
    sourcedIndex = sourced % numThreads
    if (sourced < numOfInputs) {
      poke(c.io.in.bits.word, bigWord.U)
      if (sourced < numThreads) {
        poke(c.io.in.bits.newThread, true.B)
        tags(sourced) = peek(c.io.out.bits.threadID)
      } else {
        poke(c.io.in.bits.newThread, false.B)
        poke(c.io.in.tag, tags(sourcedIndex).asUInt)
      }
