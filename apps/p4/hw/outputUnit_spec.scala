...
class outputUnit(reg_width: Int, num_regs_lg: Int, opcode_width: Int, num_threads: Int, ip_width: Int) extends Module {
  ...
  when (outReqState === 0.U) {
    ...
    io.rd_req_valid := true.B
    io.rd_req.rdAddr1 := 8.U
    io.rd_req.rdAddr2 := 9.U
    when (io.rd_req_ready) {
      when (hdr_mode < 9.U) {
        outReqState := 7.U
      } .otherwise {
    ...
    io.rd_req_valid := true.B
    io.rd_req.rdAddr1 := 10.U
    io.rd_req.rdAddr2 := 11.U
    when (io.rd_req_ready) {
      outReqState := 7.U
    }
  ...
  when (outRspState === 0.U) {
    ...
    when (io.out_ready) {
      ...
      io.rd_rsp_ready := true.B
      when (io.rd_rsp_valid) {
        out_buf := Cat(0.U, io.rd_rsp.rdData2(63, 0), io.rd_rsp.rdData1(63, 48))
        out_last := false.B
    ...
    when (io.out_ready) {
      ...
      io.rd_rsp_ready := true.B
      when (io.rd_rsp_valid) {
        out_buf := Cat(0.U, io.rd_rsp.rdData2(63, 0), io.rd_rsp.rdData1(63, 0), out_buf(79, 0))
        out_last := false.B
    ...
    when (io.out_ready) {
      ...
      io.rd_rsp_ready := true.B
      when (io.rd_rsp_valid) {
        out_buf := Cat(0.U, io.rd_rsp.rdData2(63, 0), io.rd_rsp.rdData1(63, 0), out_buf(207, 0))
        out_last := false.B
    ...
    when (io.out_ready) {
      ...
      io.rd_rsp_ready := true.B
      when (io.rd_rsp_valid) {
        out_buf := Cat(0.U, io.rd_rsp.rdData2(63, 0), io.rd_rsp.rdData1(63, 0), out_buf(335, 0))
        out_last := false.B
        when (out_hdr_mode < 10.U) {
          out_valid := true.B
          out_empty := 14.U
          outRspState := 11.U
        } .otherwise {
