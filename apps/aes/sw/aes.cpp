...
__attribute__((always_inline)) unsigned int rot_word(unsigned int word, unsigned int shift) {
    return ((word & 0xFF000000) >> 24) | ((word & 0xFFFFFF) << 8);
}

__attribute__((always_inline)) unsigned int sub_word(unsigned int word) {
    const int s_box[256] = {
...
__attribute__((always_inline)) unsigned int* key_exp(unsigned int *key, int keylen) {
    ...
    for (int i = 0; i < 4*R; i++) {
        if (i < N) {
            ...
            W[i] = W[i-N] ^ sub_word(rot_word(W[i-1], 8)) ^ r_con[i/N];
...
void primate_main() {
    ...
    PRIMATE::output<output_t>(outctx);
}
