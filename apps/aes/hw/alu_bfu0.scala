...
class alu_bfu0(conf: PrimateConfig) extends Module {

  val tag_width    = conf.TAGWIDTH
  val reg_width    = conf.REG_WIDTH
  val opcode_width = conf.OPCODE_WIDTH
  val num_threads  = conf.NUM_THREADS
  val ip_width     = conf.IP_WIDTH
