...
class alu_bfu0(tag_width: Int, reg_width: Int, opcode_width: Int, num_threads: Int, ip_width: Int) extends Module {
  ...
  class bfuIn_t extends Bundle {
    ...
    val imm    = UInt(12.W)
    val bits   = Vec(1, UInt(reg_width.W))
  }
  ...
  io.ft_out_valid := bfuInputBuf.io.deq.valid
  io.ft_out_tag := bfuInputBuf.io.deq.bits.tag
  io.ft_out_opcode := bfuInputBuf.io.deq.bits.opcode
  io.ft_out_imm := bfuInputBuf.io.deq.bits.imm
  io.ft_out_bits := bfuInputBuf.io.deq.bits.bits
  bfuInputBuf.io.deq.ready := io.ft_out_ready
  ...
  when (io.in_alu_bfu) {
    io.in_ready := bfuInputBuf.io.enq.ready
  } .otherwise {
    io.in_ready := true.B
  }
  ...
  when (alu_valid_d0) {
    ...
    io.out_valid := io.ft_in_valid
    io.out_tag := io.ft_in_tag
    io.out_flag := (io.ft_in_flag << 2)
    io.out_bits := io.ft_in_bits
