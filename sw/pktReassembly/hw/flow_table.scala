...
class flow_table_wrap(tag_width: Int) extends 
  ...
  val io = IO(new Bundle {
    ...
    val ch0_rep_data_flag                   = Output(UInt(4.W))
    ...
    val ch0_rep_data_ch0_pkt_prot           = Output(UInt(8.W))
    val ch0_rep_data_ch0_pkt_tuple_sIP      = Output(UInt(32.W))
    val ch0_rep_data_ch0_pkt_tuple_dIP      = Output(UInt(32.W))
    val ch0_rep_data_ch0_pkt_tuple_sPort    = Output(UInt(16.W))
    val ch0_rep_data_ch0_pkt_tuple_dPort    = Output(UInt(16.W))
    val ch0_rep_data_ch0_pkt_seq            = Output(UInt(32.W))
    val ch0_rep_data_ch0_pkt_len            = Output(UInt(16.W))
    val ch0_rep_data_ch0_pkt_pktID          = Output(UInt(10.W))
    val ch0_rep_data_ch0_pkt_empty          = Output(UInt(6.W))
    val ch0_rep_data_ch0_pkt_flits          = Output(UInt(5.W))
    val ch0_rep_data_ch0_pkt_hdr_len        = Output(UInt(9.W))
    val ch0_rep_data_ch0_pkt_tcp_flags      = Output(UInt(9.W))
    val ch0_rep_data_ch0_pkt_pkt_flags      = Output(UInt(3.W))
    val ch0_rep_data_ch0_pkt_pdu_flag       = Output(UInt(2.W))
    val ch0_rep_data_ch0_pkt_last_7_bytes   = Output(UInt(56.W))
...
class flowTable(tag_width: Int, reg_width: Int, opcode_width: Int, num_threads: Int, ip_width: Int) extends MultiIOModule {
  val ch0 = IO(new Bundle {
    ...
    val out_bits  = Output(UInt(518.W))
  ...
  class ch1_rep_t extends Bundle {
    val tag = UInt(tag_width.W)
  }

  val ch0_fifo = Module(new Queue(new ch0_rep_t, num_threads))
  val ch1_fifo = Module(new Queue(new ch1_rep_t, num_threads))
  ...
  ch0_fifo.io.enq.valid := ch0_rep_valid
  ch0_fifo.io.enq.bits.tag := ch0_rep_tag
  ch0_fifo.io.enq.bits.flag := ch0_rep_data.flag
  ch0_fifo.io.enq.bits.bits := Cat(ch0_rep_data.ch0_bit_map, ch0_rep_data.ch0_q.asUInt, ch0_rep_data.ch0_pkt.asUInt)
  ch0_rep_ready := true.B

  ch0.out_valid := ch0_fifo.io.deq.valid
  ch0.out_tag := ch0_fifo.io.deq.bits.tag
  ch0.out_flag := ch0_fifo.io.deq.bits.flag
  ch0.out_bits := ch0_fifo.io.deq.bits.bits
  ch0_fifo.io.deq.ready := ch0.out_ready
  ...
  ch1_fifo.io.enq.valid := ch1_rep_valid
  ch1_fifo.io.enq.bits.tag := ch1_rep_tag
  ch1_rep_ready := true.B

  ch1.out_valid := ch1_fifo.io.deq.valid
  ch1.out_tag := ch1_fifo.io.deq.bits.tag
  ch1.out_flag := 0.U
  ch1.out_bits := 0.U
  ...
  ch0_rep_data.ch0_pkt.prot                     := ft_inst.io.ch0_rep_data_ch0_pkt_prot
  ch0_rep_data.ch0_pkt.tuple.sIP                := ft_inst.io.ch0_rep_data_ch0_pkt_tuple_sIP
  ch0_rep_data.ch0_pkt.tuple.dIP                := ft_inst.io.ch0_rep_data_ch0_pkt_tuple_dIP
  ch0_rep_data.ch0_pkt.tuple.sPort              := ft_inst.io.ch0_rep_data_ch0_pkt_tuple_sPort
  ch0_rep_data.ch0_pkt.tuple.dPort              := ft_inst.io.ch0_rep_data_ch0_pkt_tuple_dPort
  ch0_rep_data.ch0_pkt.seq                      := ft_inst.io.ch0_rep_data_ch0_pkt_seq
  ch0_rep_data.ch0_pkt.len                      := ft_inst.io.ch0_rep_data_ch0_pkt_len
  ch0_rep_data.ch0_pkt.pktID                    := ft_inst.io.ch0_rep_data_ch0_pkt_pktID
  ch0_rep_data.ch0_pkt.empty                    := ft_inst.io.ch0_rep_data_ch0_pkt_empty
  ch0_rep_data.ch0_pkt.flits                    := ft_inst.io.ch0_rep_data_ch0_pkt_flits
  ch0_rep_data.ch0_pkt.hdr_len                  := ft_inst.io.ch0_rep_data_ch0_pkt_hdr_len
  ch0_rep_data.ch0_pkt.tcp_flags                := ft_inst.io.ch0_rep_data_ch0_pkt_tcp_flags
  ch0_rep_data.ch0_pkt.pkt_flags                := ft_inst.io.ch0_rep_data_ch0_pkt_pkt_flags
  ch0_rep_data.ch0_pkt.pdu_flag                 := ft_inst.io.ch0_rep_data_ch0_pkt_pdu_flag
  ch0_rep_data.ch0_pkt.last_7_bytes             := ft_inst.io.ch0_rep_data_ch0_pkt_last_7_bytes
